...
namespace ShoppingCartGrpc.Services
{
    ...
    public class ShoppingCartService : ShoppingCartProtoService.ShoppingCartProtoServiceBase
    {
        ...
        // requestStream: An IAsyncStreamReader that allows reading a stream of AddItemIntoShoppingCartRequest messages sent by the client.
        // context: The ServerCallContext that contains contextual information about the gRPC call.
        ...
        {
            // MoveNext method asynchronously moves the stream pointer to the next message in the stream. It returns true if there is another message, and false if there are no more messages in the stream.
            while (await requestStream.MoveNext())
            {
                ...
                // This line queries the database to retrieve the shopping cart associated with the username provided in the current AddItemIntoShoppingCartRequest. It uses Entity Framework Core to perform the database query. If no shopping cart is found for the given username, it throws a gRPC exception with a NotFound status, indicating that the shopping cart does not exist.
                ...
                // This line maps the NewCartItem from the current request to a new instance of ShoppingCartItem. It uses AutoMapper to perform the mapping, converting the data from the AddItemIntoShoppingCartRequest.NewCartItem to the ShoppingCartItem entity type.
                var newAddedCartItem = _mapper.Map<ShoppingCartItem>(requestStream.Current.NewCartItem);

                // This line searches for an existing cart item in the shopping cart that matches the ProductId of the newly added cart item. If an existing cart item is found, it means the product already exists in the shopping cart, and its quantity will be increased.
                var cartItem = shoppingCart.Items.FirstOrDefault(i => i.ProductId == newAddedCartItem.ProductId);
                if (null != cartItem)
                {
                    // If the product already exists in the shopping cart, its quantity is incremented by one.
                    cartItem.Quantity++;
                }
                else
                {
                    // If the product does not exist in the shopping cart, this block of code executes, indicating that the product is new to the cart.

                    // GRPC CALL DISCOUNT SERVICE -- check discount and set the item price

                    // This line makes a gRPC call to the GetDiscount method of the DiscountService to retrieve the discount information based on the DiscountCode provided in the current request.
                    var discount = await _discountService.GetDiscount(requestStream.Current.DiscountCode);

                    // After receiving the discount information, the price of the newly added cart item is adjusted by subtracting the discount amount.
